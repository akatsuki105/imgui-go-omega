...
func (me MemoryEditor) DrawContents(data []uint8, baseAddr uint) {
	C.IggMemoryEditorDrawContents(me.handle(), unsafe.Pointer(&data[0]), C.size_t(len(data)), C.size_t(baseAddr))
}
